...
fn nearest_kdtree_naive(
    ...
    if (nodes[*idx_node_nearest].pos - pos_in).norm() <= signed_distance_aabb(&pos_in, &aabb_node) {
        return;
    }
...
fn nearest_kdtree_faster(
    ...
    if i_depth % 2 == 0 {
        ...
        let aabb_west = [aabb_node[0], aabb_node[1], pos_node.x, aabb_node[3]];
        let aabb_east = [pos_node.x, aabb_node[1], aabb_node[2], aabb_node[3]];
        if signed_distance_aabb(&pos_in, &aabb_west) <= signed_distance_aabb(&pos_in, &aabb_east) {
            nearest_kdtree_naive(
                idx_node_nearest,
                pos_in,
                nodes,
        ...
        let aabb_south = [aabb_node[0], aabb_node[1], aabb_node[2], pos_node.y];
        // division in y-direction
        if signed_distance_aabb(&pos_in, &aabb_south) <= signed_distance_aabb(&pos_in, &aabb_north)
        {
            nearest_kdtree_naive(
                idx_node_nearest,
                pos_in,
